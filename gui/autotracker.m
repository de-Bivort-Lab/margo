...
if ~isempty(handles.aux_COM_list)
    expmt.hardware.COM.aux = handles.aux_COM_list(1);
end
...
if ~isempty(expmt.hardware.cam)
    if ~isempty(expmt.hardware.cam.DeviceInfo)
        ...
        elseif isfield(expmt.meta.roi,'n') && expmt.meta.roi.n && isfield(expmt.meta.ref,'im')
            expmt.meta.roi = [];
            expmt.meta.ref = [];
            expmt = rmfield(expmt.meta.vignette,'im');
...
elseif any(strcmp(expmt.meta.name,handles.parameter_subgui)) &&...
    ...
    % Execute the appropriate script for the selected experiment
        switch expmt.meta.name              
            ...
            case 'LED Y-maze'
                if ~isfield(expmt,'AUX_COM') || isempty(expmt.hardware.COM.aux)
    ...
    if isfield(expmt.meta.roi,'n') && expmt.meta.roi.n && ~keep_gui_state
        ...
        if isfield(expmt,'fields')
            for i = 1:length(expmt.meta.fields)
                expmt = rmfield(expmt,expmt.meta.fields{i});
            end
        end
...
if isfield(expmt.meta.ref,'im')
    ...
    if expmt.meta.exp_id > 1
        ...
        if isfield(expmt,'Name') && any(strcmp(expmt.meta.name,handles.parameter_subgui))
            handles.exp_parameter_pushbutton.Enable = 'on';
        end
    end
...
if strcmp(expmt.meta.source,'video') && isfield(expmt.meta.video,'vid')
    ...
    switch handles.gui_fig.UserData.ROI_mode
        ...
        if isfield(expmt.meta.roi,'centers') && size(expmt.meta.roi.centers,1) > 0
            % Enable downstream UI controls
            handles.track_thresh_slider.Enable = 'on';
            handles.accept_track_thresh_pushbutton.Enable = 'on';
            handles.reference_pushbutton.Enable = 'on';
...
for i = 1:length(handles.aux_COM_list)
    ...
    switch i
        case 1
            menu_items(i).Separator = 'on';
            menu_items(i).Checked = 'on';
            expmt.hardware.COM.aux = handles.aux_COM_list(i);
    end
end
...
switch hObject.Checked
    ...
    case 'on'                                           
        ...
        if length(expmt.hardware.COM.aux)<2
            expmt = rmfield(expmt,'AUX_COM');           % remove AUX_COM if only one in list
        else
            del = [];                                   % delete the appropriate index in list if multiple items
            for i = 1:length(expmt.hardware.COM.aux)
                if strcmp(expmt.hardware.COM.aux{i},hObject.Label)
                	del = i;
                end
            end
            expmt.hardware.COM.aux(del) = [];
        end
    ...
    case 'off'
        ...
        if isfield(expmt,'AUX_COM')
            expmt.hardware.COM.aux(length(expmt.hardware.COM.aux)+1) = {hObject.Label};   % add to the AUX_COM list
        else
            expmt.hardware.COM.aux(1) = {hObject.Label};
        end
end
...
switch hObject.Checked
    ...
    case 'off'
        ...
        if isfield(expmt.meta.roi,'n') && expmt.meta.roi.n && strcmp(handles.gui_fig.UserData.ROI_mode,'auto')
            hObject.Checked = 'on';
            hold on
            for i =1:length(expmt.meta.roi.centers)
                handles.view_menu.UserData.hBounds(i) =...
                    rectangle(handles.axes_handle,'Position',expmt.meta.roi.bounds(i,:),'EdgeColor','r');
            end
            hold off
        elseif isfield(expmt.meta.roi,'n') && expmt.meta.roi.n && strcmp(handles.gui_fig.UserData.ROI_mode,'grid')
            hObject.Checked = 'on';
        ...
        if isfield(expmt.meta.roi,'n') && expmt.meta.roi.n
            ...
            for i =1:length(expmt.meta.roi.centers)
                handles.view_menu.UserData.hOri(i) =...
                    text(handles.axes_handle,expmt.meta.roi.centers(i,1),...
                    expmt.meta.roi.centers(i,2),num2str(expmt.meta.roi.orientation(i)),...
