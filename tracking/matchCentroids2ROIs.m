        ...
        switch udat.sort_mode
          ...
          case 'bounds'  
            ...
            switch udat.ROI_mode    
                ...
                case 'auto'
                    bounds = expmt.ROI.bounds;
            end
            
            x_bounded = raw_cen(j,1) > bounds(:,1) & raw_cen(j,1) < sum(bounds(:,[1 3]),2);
            y_bounded = raw_cen(j,2) > bounds(:,2) & raw_cen(j,2) < sum(bounds(:,[2 4]),2);
            ...
            % if unsorted centroids remain assign them

            % Update last known centroid and orientations
            sorting_permutation = j(~isnan(j));
            sorting_permutation = squeeze(sorting_permutation);
            update_centroid=squeeze(~isnan(j));
            
            
            unassigned = ~ismember(1:size(raw_cen,1),sorting_permutation);
            
            if any(unassigned)

               %{ 
                % record which raw centroids are already sorted
                assigned = ~unassigned;
                
                % Define placeholder data variables equal to number ROIs
                tempCenDat=NaN(size(expmt.ROI.centers,1),2);

                % Initialize temporary centroid variables
                tempCenDat(1:size(raw_cen(unassigned,:),1),:) = raw_cen(unassigned,:);

                % Check to see if any assigned centroids are within bound
                % of their closest ROI
                tD=repmat(tempCenDat,1,1,size(expmt.ROI.centers,1));
                c=repmat(trackDat.Centroid,1,1,size(tD,1));
                c=permute(c,[3 2 1]);
                ...
                % this time find closest ROI center for each remaining centroid
                [primary_distance,j]=min(g,[],3);           
                j(isnan(tempCenDat(:,1))) = [];             % remove NaNs
                
                % remove coords that match with already assigned ROI
                no_match = find(unassigned);
                unassigned(no_match(ismember(j,find(update_centroid)))) = false;
                j(ismember(j,find(update_centroid))) = [];  
                
                % check to see if the remaining coordinates are inside the matched ROI
                x_bounded = raw_cen(unassigned,1) > bounds(j,1) &...
                    raw_cen(unassigned,1) < sum(bounds(j,[1 3]),2);
                y_bounded = raw_cen(unassigned,2) > bounds(j,2) &...
                    raw_cen(unassigned,2) < sum(bounds(j,[2 4]),2);
                in_bounds = x_bounded & y_bounded;
                j(~in_bounds) = NaN;
                ...
                % one and exclude the others
                u=unique(j(~isnan(j)));                                         % Extract the unique values of the ROIs
                dupROI=u(squeeze(histc(j,u))>1);
                dupCen=find(ismember(j,u(squeeze(histc(j,u))>1)));       % Find the indices of duplicate ROIs

                % Calculate pairwise distances between duplicate ROIs and temp centroids
                % using the same method above
                tD=repmat(tempCenDat(dupCen,:),1,1,size(trackDat.Centroid,1));
                c=repmat(trackDat.Centroid,1,1,size(tempCenDat(dupCen,:),1));
                c=permute(c,[3 2 1]);
                g=sqrt(dot((c-tD),(tD-c),2));
                g=abs(g);
                [~,k]=min(g);
                j(ismember(j,dupROI))=NaN;
                j(dupCen(k(dupROI)))=dupROI;  
                
                % new updates
                ROI_list = find(update_centroid);
                Cen_list = sorting_permutation;
                
                new_assigned = unassigned';

                
                new_assigned(new_assigned) = ~isnan(j);
                new_ROI_list = [ROI_list;j(~isnan(j))];
                new_Cen_list = [Cen_list;find(new_assigned)];
                [~,p] = sort(new_ROI_list);
                sorting_permutation = new_Cen_list(p);
                update_centroid = ismember(1:size(expmt.ROI.centers,1),new_ROI_list)'; 
              %}
...
function boundsAssignROI(cen,b)
...
function rois = findSingleAssignment(rois,cen_ct)                 
    
% for each centroid with duplicate assignments, 
% check if any assigned ROI has another centroid
% assigned. If so, assign the centroid to that ROI.
