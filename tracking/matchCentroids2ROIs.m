    ...
    if size(raw_cen,1)>0 
        ...
        elseif strcmp(udat.sort_mode,'bounds')
            ...
            if any(unassigned)
                ...
                c=repmat(trackDat.Centroid,1,1,size(expmt.ROI.centers,1));
                c=permute(c,[3 2 1]);
                ...
                x_bounded = raw_cen(unassigned,1) > expmt.ROI.bounds(j,1) &...
                    raw_cen(unassigned,1) < sum(expmt.ROI.bounds(j,[1 3]),2);
                y_bounded = raw_cen(unassigned,2) > expmt.ROI.bounds(j,2) &...
                    raw_cen(unassigned,2) < sum(expmt.ROI.bounds(j,[2 4]),2);
                in_bounds = x_bounded & y_bounded;
                j(~in_bounds) = NaN;
