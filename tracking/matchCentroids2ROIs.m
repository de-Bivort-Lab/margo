function [varargout]=matchCentroids2ROIs(varargin)
...
vars=parseinputvars(varargin);

% Define placeholder data variables equal to number ROIs
tempCenDat=NaN(size(vars.sort_coords_primary,1),2);

% Initialize temporary centroid variables
tempCenDat(1:size(vars.cenDat,1),:)=vars.cenDat;
...
tD=repmat(tempCenDat,1,1,size(vars.sort_coords_primary,1));
c=repmat(vars.sort_coords_primary,1,1,size(tempCenDat,1));
c=permute(c,[3 2 1]);
...
sorting_permutation=[];
update_centroid=boolean(zeros(size(vars.sort_coords_primary,1),1));
