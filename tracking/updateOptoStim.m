...
%       presentations, and are not already being presented with a stimulus
        ...
        if any(trackDat.StimStatus)
            ...
            p_rotim = ...
                imrotate(expmt.stim.im, expmt.stim.angle, 'bilinear', 'crop');
            p_rotim = ...
                p_rotim(expmt.stim.bounds(2):expmt.stim.bounds(4),...
                    expmt.stim.bounds(1):expmt.stim.bounds(3));
            n_rotim = ...
                imrotate(expmt.stim.im, -expmt.stim.angle, 'bilinear', 'crop');
            n_rotim = ...
                n_rotim(expmt.stim.bounds(2):expmt.stim.bounds(4),...
                    expmt.stim.bounds(1):expmt.stim.bounds(3));

            % Calculate the displacement from the ROI center in projector space
            p_cen = NaN(sum(trackDat.StimStatus),2);
            p_cen(:,1) = ...
                expmt.projector.Fx(trackDat.centroid(trackDat.StimStatus,1),...
                    trackDat.centroid(trackDat.StimStatus,2));
            p_cen(:,2) = ...
                expmt.projector.Fy(trackDat.centroid(trackDat.StimStatus,1),...
                    trackDat.centroid(trackDat.StimStatus,2));
            p_dist = ...
                [p_cen(:,1) - expmt.stim.centers(trackDat.StimStatus,1),...
            ...
            if any(trackDat.Texture(trackDat.StimStatus))
                Screen('DrawTextures', expmt.hardware.screen.window, ...
                    ...
                    expmt.stim.corners(...
                    trackDat.StimStatus & trackDat.Texture,:)',...
                    [],[],[],[],[],[]);
            end
            if any(~trackDat.Texture(trackDat.StimStatus))
                Screen('DrawTextures', expmt.hardware.screen.window, ...
                    expmt.stim.pinTex_neg,...
                    src_rects(~trackDat.Texture(trackDat.StimStatus),:)', ...
                    expmt.stim.corners(trackDat.StimStatus & ...
                    ~trackDat.Texture,:)', [],[], [], [],[], []);
            end
        ...
        % Turn off stimuli that have exceed the display duration
          stim_OFF = ...
              trackDat.t-expmt.stim.t >= expmt.parameters.stim_duration &...
                trackDat.StimStatus;
