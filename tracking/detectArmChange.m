...
[d0,curr_arm_0] = ...
    min(permute(d(~expmt.ROI.orientation,:,1:3),[3 2 1]));    % Closest arm for right-side down maze
[d1,curr_arm_1] = ...
    min(permute(d(expmt.ROI.orientation,:,4:6),[3 2 1]));     % Closest arm for right-side up maze
curr_arm(~expmt.ROI.orientation) = curr_arm_0;
curr_arm(expmt.ROI.orientation) = curr_arm_1;
curr_arm=squeeze(curr_arm);

d=NaN(size(trackDat.arm,1),1);
d(~expmt.ROI.orientation) = d0;
d(expmt.ROI.orientation) = d1;
