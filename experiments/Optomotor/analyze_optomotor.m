...
if isfield(expmt,'Contrast')
    ...
    dim = ceil(length(expmt.sweep.contrasts)/3);
    figure();
...
if isfield(expmt,'AngularVel')
    ...
    dim = ceil(length(expmt.sweep.ang_vel)/3);
...
if isfield(expmt,'SpatialFreq')
    ...
    dim = ceil(length(expmt.sweep.spatial_freq)/3);
    ...
    for i = 1:length(expmt.sweep.spatial_freq)
        ...
        % create plots
        titstr = ['angular velocity = ' num2str(expmt.sweep.spatial_freq(i))];
        plotOptoTraces(da,active,expmt.parameters.stim_int,'title',titstr,'Ylim',[llim ulim]);
