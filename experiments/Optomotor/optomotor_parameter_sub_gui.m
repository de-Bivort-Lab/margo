function varargout = optomotor_parameter_sub_gui(varargin)
% optomotor_parameter_sub_gui MATLAB code for optomotor_parameter_sub_gui.fig
%      optomotor_parameter_sub_gui, by itself, creates a new optomotor_parameter_sub_gui or raises the existing
%      singleton*.
%
%      H = optomotor_parameter_sub_gui returns the handle to a new optomotor_parameter_sub_gui or the handle to
%      the existing singleton*.
%
%      optomotor_parameter_sub_gui('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in optomotor_parameter_sub_gui.M with the given input arguments.
%
%      optomotor_parameter_sub_gui('Property','Value',...) creates a new optomotor_parameter_sub_gui or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before optomotor_parameter_sub_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to optomotor_parameter_sub_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help optomotor_parameter_sub_gui
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @optomotor_parameter_sub_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @optomotor_parameter_sub_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before optomotor_parameter_sub_gui is made visible.
function optomotor_parameter_sub_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to optomotor_parameter_sub_gui (see VARARGIN)
    ...
    if isfield(expmt,'sweep')
        ...
        if isfield(expmt.sweep,'interval')
            ...
            handles.output.parameters.sweep.interval = str2double(handles.edit_block_interval.String);
        end
    else
...
% UIWAIT makes optomotor_parameter_sub_gui wait for user response (see UIRESUME)
...
function varargout = optomotor_parameter_sub_gui_OutputFcn(hObject, eventdata, handles) 
...
handles.output.hardware.projector.reg_params.screen_num = handles.scr_popupmenu.Value-1;
guidata(hObject,handles);
...
handles.output.parameters.sweep = sweep;
guidata(hObject,handles);
...
handles.output.parameters.sweep.interval = str2double(hObject.String);
guidata(hObject,handles);
