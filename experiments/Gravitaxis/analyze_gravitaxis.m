...
expmt.Activity.mask = trackProps.speed > 0.8;
expmt.Speed.y = [zeros(1,expmt.nTracks);squeeze(diff(expmt.Centroid.data(:,2,:)))];
expmt.Speed.y(~expmt.Activity.mask) = NaN;
posY = expmt.Speed.y > 0;
negY = expmt.Speed.y < 0;
s=expmt.Speed.y;
s(negY) = NaN;
expmt.Gravitaxis.pos_vel = nanmean(s);
s=expmt.Speed.y;
s(posY) = NaN;
expmt.Gravitaxis.neg_vel = nanmean(s);
expmt.Gravitaxis.index = (expmt.Gravitaxis.pos_vel + expmt.Gravitaxis.neg_vel)./...
    (expmt.Gravitaxis.pos_vel + abs(expmt.Gravitaxis.neg_vel));

expmt = scoreGraviChoice(expmt,trackProps);
