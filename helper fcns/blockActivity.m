...
s = permute(s,[3 1 2]);
[ac] = autocorr(s(:),250);
lag_thresh = find(diff(ac)>-0.001,1) + 1;
...
transitions = diff(moving,1,3);
transitions = cat(3,zeros(size(transitions,1),1,1),transitions);
transitions = num2cell(transitions,3);
